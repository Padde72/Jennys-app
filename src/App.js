...
// --- SVG Icons (defined outside the main component to prevent re-creation) ---
const HeaderIcon = () => (
...
const App = () => {
  ...
  // Check for persisted login session on component mount
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(authInstance, (user) => {
        if (user) {
            ...
            setIsFirebaseReady(true); // Firebase is ready once we have a user
        } else {
            signInAnonymously(authInstance).catch(error => {
                console.error("Anonymous sign-in failed:", error);
                setIsFirebaseReady(true); // Still ready, but auth failed
            });
        }
    });
  ...
  useEffect(() => {
    if (isQrModalOpen && selectedProductForPurchase) {
      const attemptToGenerateQr = (retries = 20) => {
        const qrcodeDiv = document.getElementById("qrcode");
        if (retries <= 0) {
            if (qrcodeDiv) qrcodeDiv.innerHTML = "<p class='text-red-500'>Kunde inte ladda QR-kod. Ladda om sidan och försök igen.</p>";
            return;
        }
        if (qrcodeDiv && typeof window.QRCode !== 'undefined') {
            ...
            setTimeout(() => attemptToGenerateQr(retries - 1), 100);
        }
      };
      attemptToGenerateQr();
    }
  ...
  const handleLoginSubmit = (event) => {
    ...
    if (password === OWNER_PASSWORD) {
        ...
        localStorage.setItem('jennysRecyclingAuth', JSON.stringify(sessionData));
  ...
  const handleImageSelection = async (event) => {
    ...
    const spaceLeft = 3 - currentImageCount;

    if (allowedFiles.length > spaceLeft) {
        setUploadMessage(`Du kan bara lägga till ${spaceLeft} bild(er) till. De första ${spaceLeft} har valts.`);
        allowedFiles = allowedFiles.slice(0, spaceLeft);
    } else {
        setUploadMessage('');
    }
    ...
    try {
        ...
        setUploadMessage('Kunde inte bearbeta bilderna. Försök med andra.');
    } finally {
  ...
  const handleProductUpload = async (event) => {
    ...
    if (!db || !userId || !storage) {
      setUploadMessage('Ett fel uppstod, databasen är inte redo. Försök igen om en stund.');
      return;
    }
    setIsUploading(true);
    ...
    try {
      ...
      console.error("Full error during product upload: ", e);
      if (e.code) {
          switch (e.code) {
              case 'storage/unauthorized':
                  setUploadMessage('Behörighetsfel. Kontrollera att dina Storage-regler i Firebase tillåter uppladdning.');
                  break;
              case 'storage/canceled':
                  setUploadMessage('Uppladdningen avbröts.');
                  break;
              default:
                  setUploadMessage(`Ett oväntat fel uppstod: ${e.code}`);
                  break;
          }
      } else {
          setUploadMessage('Ett fel uppstod. Kontrollera din internetanslutning och försök igen.');
      }
    } finally {
        setIsUploading(false);
    }
  ...
  const handleAddressSubmit = async (event) => {
    ...
    if (!db || !userId || !selectedProductForPurchase) {
      setUploadMessage('Kunde inte skicka beställning. Kontrollera din anslutning och försök igen.');
      return;
    }
    ...
    try {
        ...
        if (isMobile) {
            const paymentData = { version: 1, payee: { value: swishNumber }, amount: { value: price }, message: { value: `Köp: ${productName}`, editable: false } };
            const encodedData = encodeURIComponent(JSON.stringify(paymentData));
            window.open(`swish://payment?data=${encodedData}`, '_blank');
        } else {
            setIsQrModalOpen(true);
        }
    } catch (error) {
        console.error("Error processing order: ", error);
        setUploadMessage('Ett fel uppstod. Beställningen kunde inte slutföras.');
    ...
    <>
          ...
          <form onSubmit={handleProductUpload}>
              <div className="mb-4">
                ...
                {imagePreviews.length < 3 && (
                    <div className="mt-4 flex flex-col sm:flex-row gap-4">
                        <button type="button" onClick={() => fileInputRef.current.click()} className="w-full bg-cyan-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-cyan-500 flex-1">
                            {imagePreviews.length === 0 ? 'Välj från bibliotek' : 'Lägg till från bibliotek'}
                        </button>
                        <button type="button" onClick={() => cameraInputRef.current.click()} className="w-full bg-gray-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-gray-500 flex-1">
                            {imagePreviews.length === 0 ? 'Ta bild med kameran' : 'Ta en till bild'}
                        </button>
                    </div>
                )}
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                    ...
                    {productName && (
                        <button type="button" onClick={() => setProductName('')} className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-white" aria-label="Rensa produktnamn">
                            <ClearIcon />
                        </button>
                    )}
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                    ...
                    {productDescription && (
                         <button type="button" onClick={() => setProductDescription('')} className="absolute top-0 right-0 mt-3 mr-3 text-gray-400 hover:text-white" aria-label="Rensa beskrivning">
                            <ClearIcon />
                        </button>
                    )}
                </div>
      ...
      <Modal isOpen={isAddressModalOpen} onClose={() => setIsAddressModalOpen(false)} title="Ange leveransadress">
          <form onSubmit={handleAddressSubmit}>
            <div className="space-y-4">
                <div><label htmlFor="name" className="block text-gray-300 font-semibold mb-2">Namn:</label><input type="text" id="name" name="name" placeholder="För- och efternamn" className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" required /></div>
                <div><label htmlFor="street" className="block text-gray-300 font-semibold mb-2">Gatuadress:</label><input type="text" id="street" name="street" placeholder="Gatunamn och nummer" className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" required /></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label htmlFor="zip" className="block text-gray-300 font-semibold mb-2">Postnummer:</label><input type="text" id="zip" name="zip" placeholder="T.ex. 123 45" className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" required /></div>
                    <div><label htmlFor="city" className="block text-gray-300 font-semibold mb-2">Ort:</label><input type="text" id="city" name="city" placeholder="T.ex. Stockholm" className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" required /></div>
                </div>
                <div><label htmlFor="phone" className="block text-gray-300 font-semibold mb-2">Telefonnummer:</label><input type="tel" id="phone" name="phone" placeholder="T.ex. 070-123 45 67" className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" required /></div>
      ...
      <Modal isOpen={isLoginModalOpen} onClose={() => setIsLoginModalOpen(false)} title="Logga in">
          <form onSubmit={handleLoginSubmit}>
            <div className="mb-4">
              ...
              <input type="password" id="password" name="password" className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" required />
            </div>
      ...
      <Modal isOpen={isCategorySelectModalOpen} onClose={() => setIsCategorySelectModalOpen(false)} title="Välj kategori">
          <div className="grid grid-cols-2 gap-4">
            {categoryButtons.map(cat => (
              <button key={cat.dataCategory} onClick={() => handleCategorySelectForUpload(cat.dataCategory)} className="bg-gray-700 text-white font-semibold py-3 px-4 rounded-lg hover:bg-gray-600 hover:border-cyan-500 border-2 border-gray-700 transition-all text-sm sm:text-base text-center">{cat.name}</button>
            ))}
          </div>
      </Modal>

      <Modal isOpen={isDeleteConfirmOpen} onClose={handleCloseDeleteConfirm} title="Bekräfta borttagning">
          <p className="text-gray-300 mb-6">Är du säker på att du vill ta bort den här produkten permanent? Detta kan inte ångras.</p>
      ...
      <div className="fixed bottom-5 right-5 z-50">
          <button 
            onClick={() => { isOwnerAuthenticated ? handleViewChange('orders') : setIsLoginModalOpen(true); }} 
            className="flex items-center gap-2 bg-gray-800 text-white font-bold py-3 px-4 rounded-full shadow-lg hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-600 focus:ring-opacity-50 transition-all duration-200 border border-gray-600"
          >
            <OrdersIcon />
